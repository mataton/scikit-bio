...
if sys.version_info.major != 3:
    raise RuntimeError(
        'scikit-bio can only be used with Python 3. You are '
    ...
    def get_items(self, names):
        ...
        def fix_item(display_name, sig, summary, real_name):
            ...
            if display_name in specials:
                ...
                obj = autosummary.import_by_name(
                    display_name, prefixes=prefixes
                )
...
def linkcode_resolve(domain, info):
    ...
    if 'dev' in skbio.__version__:
        return (
            'http://github.com/biocore/scikit-bio/blob/master/skbio/%s%s'
            % (
                fn,
                linespec,
            )
