...
class TestGrammaredSequence(TestCase):
    ...
    def test_gap_chars_and_definites_share(self):
        with self.assertRaisesRegex(
            ...
            class GrammaredSequenceGapInDefiniteMap(ExampleGrammaredSequence):
                @classproperty
                def degenerate_map(cls):
                    return {'X': set('AB')}
    ...
    def test_init_default_parameters(self):
        ...
        assert_data_frame_almost_equal(
            seq.positional_metadata, pd.DataFrame(index=range(8))
        )
        self.assertEqual(seq.interval_metadata, IntervalMetadata(8))
    ...
    def test_init_nondefault_parameters(self):
        ...
        assert_data_frame_almost_equal(
            seq.positional_metadata, pd.DataFrame({'quality': range(8)})
        )
    ...
    def test_init_lowercase_invalid_keys(self):
        for invalid_key in ((), [], 2):
            ...
            with self.assertRaisesRegex(
                ...
                'a bool or string, but got %s' % invalid_type,
            ):
    ...
    def test_degenerate_chars(self):
        ...
        self.assertEqual(
            ExampleGrammaredSequence('').degenerate_chars, expected
        )
    ...
    def test_nondegenerate_chars(self):
        expected = set('ABC')
        self.assertEqual(
            ExampleGrammaredSequence.nondegenerate_chars, expected
        )
    ...
    def test_definite_chars(self):
        expected = set('ABC')
        self.assertEqual(ExampleGrammaredSequence.definite_chars, expected)
    ...
    def test_gaps(self):
        ...
        self.assertIs(
            ExampleGrammaredSequence('').gaps().dtype, np.dtype('bool')
        )
        npt.assert_equal(
            ExampleGrammaredSequence('ABCXBZYABC').gaps(),
            np.zeros(10).astype(bool),
        )

        npt.assert_equal(
            ExampleGrammaredSequence('.-.-.').gaps(), np.ones(5).astype(bool)
        )

        npt.assert_equal(
            ExampleGrammaredSequence('A.B-C.X-Y.').gaps(),
            np.array([0, 1] * 5, dtype=bool),
        )

        npt.assert_equal(
            ExampleGrammaredSequence('AB.AC.XY-').gaps(),
            np.array([0, 0, 1] * 3, dtype=bool),
        )

        npt.assert_equal(
            ExampleGrammaredSequence('A.BC.-').gaps(),
    ...
    def test_degenerates(self):
        self.assertIs(
            type(ExampleGrammaredSequence('').degenerates()), np.ndarray
        )
        self.assertIs(
            ExampleGrammaredSequence('').degenerates().dtype, np.dtype('bool')
        )

        npt.assert_equal(
            ExampleGrammaredSequence('ABCBC-.AB.').degenerates(),
            np.zeros(10).astype(bool),
        )

        npt.assert_equal(
            ExampleGrammaredSequence('ZYZYZ').degenerates(),
            np.ones(5).astype(bool),
        )

        npt.assert_equal(
            ExampleGrammaredSequence('AX.Y-ZBXCZ').degenerates(),
            np.array([0, 1] * 5, dtype=bool),
        )

        npt.assert_equal(
            ExampleGrammaredSequence('ABXACY.-Z').degenerates(),
            np.array([0, 0, 1] * 3, dtype=bool),
        )

        npt.assert_equal(
            ExampleGrammaredSequence('AZBCXY').degenerates(),
            np.array([0, 1, 0, 0, 1, 1], dtype=bool),
        )

    def test_has_degenerates(self):
        self.assertIs(
            type(ExampleGrammaredSequence('').has_degenerates()), bool
        )
        self.assertIs(
            type(ExampleGrammaredSequence('X').has_degenerates()), bool
        )
    ...
    def test_nondegenerates(self):
        self.assertIs(
            type(ExampleGrammaredSequence('').nondegenerates()), np.ndarray
        )
        self.assertIs(
            ExampleGrammaredSequence('').nondegenerates().dtype,
            np.dtype('bool'),
        )
        ...
        npt.assert_equal(
            ExampleGrammaredSequence('ABABA').nondegenerates(),
            np.ones(5).astype(bool),
        )
        ...
        npt.assert_equal(
            ExampleGrammaredSequence('YB.-AC').nondegenerates(),
            np.array([0, 1, 0, 0, 1, 1], dtype=bool),
        )

    def test_definites(self):
        self.assertIs(
            type(ExampleGrammaredSequence('').definites()), np.ndarray
        )
        self.assertIs(
            ExampleGrammaredSequence('').definites().dtype, np.dtype('bool')
        )
        ...
        npt.assert_equal(
            ExampleGrammaredSequence('ABABA').definites(),
            np.ones(5).astype(bool),
        )
        ...
        npt.assert_equal(
            ExampleGrammaredSequence('YB.-AC').definites(),
            np.array([0, 1, 0, 0, 1, 1], dtype=bool),
        )
    ...
    def test_has_nondegenerates(self):
        self.assertIs(
            type(ExampleGrammaredSequence('').has_nondegenerates()), bool
        )
        self.assertIs(
            type(ExampleGrammaredSequence('A').has_nondegenerates()), bool
        )
    ...
    def test_has_definites(self):
        self.assertIs(type(ExampleGrammaredSequence('').has_definites()), bool)
        self.assertIs(
            type(ExampleGrammaredSequence('A').has_definites()), bool
        )

        self.assertFalse(ExampleGrammaredSequence('').has_definites())
        self.assertFalse(ExampleGrammaredSequence('X-.YZ').has_definites())

        self.assertTrue(ExampleGrammaredSequence('C').has_definites())
        self.assertTrue(ExampleGrammaredSequence('.XYZ-ABC').has_definites())
    ...
    def test_degap(self):
        ...
        self.assertEqual(
            ExampleGrammaredSequence(
                'ABCXYZ', positional_metadata={'qual': np.arange(6)}, **kw
            ).degap(),
            ExampleGrammaredSequence(
                'ABCXYZ', positional_metadata={'qual': np.arange(6)}, **kw
            ),
        ...
        self.assertEqual(
            ExampleGrammaredSequence(
                'ABC-XYZ', positional_metadata={'qual': np.arange(7)}, **kw
            ).degap(),
            ExampleGrammaredSequence(
                'ABCXYZ',
        ...
        self.assertEqual(
            ExampleGrammaredSequence(
                '.-ABC-XYZ.', positional_metadata={'qual': np.arange(10)}, **kw
            ).degap(),
            ExampleGrammaredSequence(
                'ABCXYZ',
    ...
    def test_expand_degenerates_all_degens(self):
        exp = [
            ExampleGrammaredSequence('ABA'),
            ExampleGrammaredSequence('ABC'),
            ExampleGrammaredSequence('ACA'),
            ExampleGrammaredSequence('ACC'),
            ExampleGrammaredSequence('BBA'),
            ExampleGrammaredSequence('BBC'),
            ExampleGrammaredSequence('BCA'),
            ExampleGrammaredSequence('BCC'),
        ]
        # Sort based on sequence string, as order is not guaranteed.
        obs = sorted(
            ExampleGrammaredSequence('XYZ').expand_degenerates(), key=str
        )
        self.assertEqual(obs, exp)

    def test_expand_degenerates_with_metadata(self):
        kw = {
            'metadata': {'id': 'some_id', 'description': 'some description'},
            'positional_metadata': {
                'quality': np.arange(3),
            },
        }
        exp = [
            ExampleGrammaredSequence('ABA', **kw),
            ExampleGrammaredSequence('ABC', **kw),
            ExampleGrammaredSequence('BBA', **kw),
            ExampleGrammaredSequence('BBC', **kw),
        ]
        obs = sorted(
            ExampleGrammaredSequence('XBZ', **kw).expand_degenerates(), key=str
        )
        self.assertEqual(obs, exp)
    ...
    def test_to_regex_with_degens(self):
        ...
        regex = seq.to_regex()
        self.assertFalse(
            any(regex.match(s) is None for s in 'ABA ABC ACA ACC'.split())
        )
        self.assertTrue(
            all(regex.match(s) is None for s in 'CBA BBA ABB AAA'.split())
        )
    ...
    def test_to_regex_within_capture(self):
        ...
        for ref in 'ABA BBB CCA'.split():
            self.assertFalse(
                any(len(match.groups()) == 1 for match in regex.finditer(ref))
            )

        for ref in 'ABC BBC ACC'.split():
            self.assertTrue(
                all(len(match.groups()) == 1 for match in regex.finditer(ref))
            )
