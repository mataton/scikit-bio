...
class TestProtein(unittest.TestCase):
    ...
    def test_stops(self):
        ...
        npt.assert_array_equal(
            Protein('PAW').stops(), np.array([False, False, False])
        )

        npt.assert_array_equal(
            Protein('PAW*').stops(), np.array([False, False, False, True])
        )

        npt.assert_array_equal(
            Protein('P*W*').stops(), np.array([False, True, False, True])
        )

        npt.assert_array_equal(
            Protein('****').stops(), np.array([True, True, True, True])
        )
    ...
    def test_motif_n_glycosylation(self):
        ...
        self.assertEqual(
            list(seq.find_motifs('N-glycosylation')), [slice(4, 8)]
        )
