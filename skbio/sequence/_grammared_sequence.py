...
class GrammaredSequence(Sequence, metaclass=GrammaredSequenceMeta):
    ...
    @classproperty
    @deprecated(
        as_of='0.5.0', until='0.6.0', reason='Renamed to definite_chars'
    )
    ...
    def _validate(self):
        ...
        if np.any(invalid_characters):
            bad = list(
                np.where(invalid_characters > 0)[0]
                .astype(np.uint8)
                .view('|S1')
            )
            raise ValueError(
    ...
    @deprecated(
        as_of='0.5.0', until='0.6.0', reason='Renamed to has_definites'
    )
    ...
    def to_regex(self, within_capture=False):
        ...
        for base in str(self):
            if base in self.degenerate_chars:
                regex_parts.append(
                    '[{0}]'.format(''.join(self.degenerate_map[base]))
                )
            else:
