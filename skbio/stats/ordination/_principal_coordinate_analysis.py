...
def pcoa(
    distance_matrix, method='eigh', number_of_dimensions=0, inplace=False
):
    ...
    if np.any(eigvals < 0):
        warn(
            ...
            ' {0} and the largest is {1}.'.format(
                eigvals.min(), eigvals.max()
            ),
            RuntimeWarning,
        )
    ...
    if use_power_method:
        ...
        H = hstack(
            (
                H,
                dot(
                    centered_distance_matrix, dot(centered_distance_matrix, H)
                ),
            )
        )
...
def pcoa_biplot(ordination, y):
    ...
    if (
        ...
        and ordination.short_method_name != 'PCoA'
    ):
        raise ValueError(
            'This biplot computation can only be performed in a '
            'PCoA matrix.'
        )

    if set(y.index) != set(ordination.samples.index):
        raise ValueError(
            'The eigenvectors and the descriptors must describe '
            'the same samples.'
        )
