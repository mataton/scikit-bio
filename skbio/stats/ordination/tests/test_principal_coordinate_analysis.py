...
class TestPCoA(TestCase):
    ...
    def test_simple(self):
        ...
        expected_results = OrdinationResults(
            ...
            proportion_explained=pd.Series(
                proportion_explained, index=axis_labels
            ),
        )
    ...
    def test_fsvd_inplace(self):
        ...
        expected_results = pcoa(
            dm1, method='eigh', number_of_dimensions=3, inplace=True
        )

        results = pcoa(
            dm2, method='fsvd', number_of_dimensions=3, inplace=True
        )
    ...
    def test_fsvd(self):
        ...
        expected_results = pcoa(
            dm1, method='eigh', number_of_dimensions=3, inplace=False
        )

        results = pcoa(
            dm2, method='fsvd', number_of_dimensions=3, inplace=False
        )

        results_inplace = pcoa(
            dm2, method='fsvd', number_of_dimensions=3, inplace=True
        )
        ...
        results2 = pcoa(
            dm3, method='fsvd', number_of_dimensions=0, inplace=False
        )
        expected_results2 = pcoa(
            dm3,
            method='fsvd',
        ...
        with self.assertWarnsRegex(
            RuntimeWarning, r'no value for number_of_dimensions'
        ):
    ...
    def test_extensive(self):
        ...
        expected_results = OrdinationResults(
            ...
            eigvals=pd.Series(eigvals, index=axis_labels),
            samples=pd.DataFrame(
                samples, index=sample_ids, columns=axis_labels
            ),
            proportion_explained=pd.Series(
                proportion_explained, index=axis_labels
            ),
