...
class TestRDAResults(TestCase):
    ...
    def test_scaling1(self):
        ...
        sample_constraints = pd.DataFrame(
            np.loadtxt(get_data_path('example2_sample_constraints_scaling1'))
        )

        # Load data as computed with vegan 2.0-8
        vegan_features = pd.DataFrame(
            np.loadtxt(get_data_path('example2_species_scaling1_from_vegan')),
            index=self.feature_ids,
            columns=self.pc_ids,
        )

        vegan_samples = pd.DataFrame(
            np.loadtxt(get_data_path('example2_site_scaling1_from_vegan')),
            index=self.sample_ids,
            columns=self.pc_ids,
        )
        ...
        mat = np.loadtxt(get_data_path('example2_biplot_scaling1'))
        cropped_pc_ids = self.pc_ids[: mat.shape[1]]
        biplot_scores = pd.DataFrame(
            mat, index=self.env_ids, columns=cropped_pc_ids
        )
        ...
        assert_ordination_results_equal(
            scores, exp, ignore_directionality=True, decimal=6
        )
    ...
    def test_scaling2(self):
        ...
        sample_constraints = pd.DataFrame(
            np.loadtxt(get_data_path('example2_sample_constraints_scaling2'))
        )

        # Load data as computed with vegan 2.0-8
        vegan_features = pd.DataFrame(
            np.loadtxt(get_data_path('example2_species_scaling2_from_vegan')),
            index=self.feature_ids,
            columns=self.pc_ids,
        )

        vegan_samples = pd.DataFrame(
            np.loadtxt(get_data_path('example2_site_scaling2_from_vegan')),
            index=self.sample_ids,
            columns=self.pc_ids,
        )
...
class TestRDAResults_biplot_score(TestCase):
    ...
    def test_biplot_score(self):
        ...
        assert_ordination_results_equal(
            rda_, exp, ignore_directionality=True, decimal=6
        )
