...
def alr_inv(mat, denominator_idx=0):
    ...
    if mat.ndim == 2:
        mat_idx = np.insert(
            mat, denominator_idx, np.repeat(0, mat.shape[0]), axis=1
        )
...
def ancom(
    ...
    if len(mat) != table_index_len or len(cats) != grouping_index_len:
        raise ValueError(
            '`table` index and `grouping` ' 'index must be consistent.'
        )
    ...
    if len(percentiles) == 0:
        ...
        columns = pd.MultiIndex.from_tuples(
            columns, names=['Percentile', 'Group']
        )
...
def _gram_schmidt_basis(n):
    ...
    for j in range(n - 1):
        i = j + 1
        e = np.array([(1 / i)] * i + [-1] + [0] * (n - i - 1)) * np.sqrt(
            i / (i + 1)
        )
        basis[:, j] = e
    return basis.T
...
def _check_orthogonality(basis):
    ...
    basis = np.atleast_2d(basis)
    if not np.allclose(
        basis @ basis.T, np.identity(len(basis)), rtol=1e-4, atol=1e-6
    ):
