...
class PowerAnalysisTest(TestCase):
    def setUp(self):
        ...
        # test.
        self.samps = [np.ones((10)) / 10.0, np.ones((10))]
    ...
    def test_subsample_power_defaults(self):
        test_p, test_c = subsample_power(
            self.f, self.pop, num_iter=10, num_runs=5
        )
    ...
    def test_subsample_power_counts(self):
        test_p, test_c = subsample_power(
            self.f, samples=self.pop, num_iter=10, num_runs=2, min_counts=5
        )
    ...
    def test_subsample_paired_power(self):
        ...
        test_p, test_c = subsample_paired_power(
            self.meta_f,
            meta=self.meta,
            cat=cat,
    ...
    def test__get_min_size_strict(self):
        known = 5
        test = _get_min_size(
            self.meta, 'INT', ['ABX', 'SEX'], ['Y', 'N'], True
        )
    ...
    def test__get_min_size_relaxed(self):
        known = 5
        test = _get_min_size(
            self.meta, 'INT', ['ABX', 'SEX'], ['Y', 'N'], False
        )
    ...
    def test__compare_distributions_sample_counts_error(self):
        with self.assertRaises(ValueError):
            _compare_distributions(
                self.f, [self.pop[0][:5], self.pop[1]], 1, counts=25
            )
    ...
    def test__compare_distributions_matched_mode(self):
        ...
        test = _compare_distributions(
            self.f, self.pop, self.num_p, mode='matched', num_iter=100
        )
    ...
    def test__compare_distributions_draw_mode(self):
        ...
        with self.assertRaises(ValueError):
            _check_subsample_power_inputs(
                self.f, self.pop, draw_mode, self.num_p
            )
    ...
    def test__compare_distributions_multiple_returns(self):
        ...
        test = _compare_distributions(
            f, self.pop, 3, mode='matched', num_iter=3
        )
    ...
    def test_check_subsample_power_inputs_ratio(self):
        with self.assertRaises(ValueError):
            _check_subsample_power_inputs(
                self.f, self.samps, ratio=np.array([1, 2, 3])
            )
    ...
    def test_check_sample_power_inputs(self):
        ...
        test_ratio, test_num_p, test_counts = _check_subsample_power_inputs(
            self.f, self.samps, counts_interval=2, max_counts=10
        )
    ...
    def test__calculate_power_curve_default(self):
        ...
        known = np.array(
            [0.509, 0.822, 0.962, 0.997, 1.000, 1.000, 1.000, 1.000, 1.000]
        )
        # Generates the test values
        test = _calculate_power_curve(
            self.f, self.pop, self.num_samps, num_iter=100
        )
    ...
    def test__calculate_power_curve_alpha(self):
        ...
        known = np.array(
            [0.31, 0.568, 0.842, 0.954, 0.995, 1.000, 1.000, 1.000, 1.000]
        )

        # Generates the test values
        test = _calculate_power_curve(
            self.f, self.pop, self.num_samps, alpha=0.01, num_iter=100
        )
    ...
    def test__calculate_power_curve_ratio(self):
        # Sets the know output
        known = np.array(
            [0.096, 0.333, 0.493, 0.743, 0.824, 0.937, 0.969, 0.996, 0.998]
        )
    ...
    def test_paired_subsample_undefined(self):
        ...
        test_array = paired_subsamples(
            self.meta, cat, control_cats, order=order
        )
    ...
    def test_paired_subsamples_not_strict(self):
        ...
        test_array = paired_subsamples(
            self.meta, cat, control_cats, strict_match=False
        )
    ...
    def test__identify_sample_groups(self):
        ...
        test_pairs, test_index = _identify_sample_groups(
            self.meta,
            'INT',
            ['SEX', 'AGE'],
            order=['N', 'Y'],
        ...
        self.assertEqual(
            sorted(known_pairs.values()), sorted(test_pairs.values())
        )
    ...
    def test__draw_paired_samples(self):
        ...
        test_samps = _draw_paired_samples(
            self.meta_pairs, self.pair_index, num_samps
        )
