...
class BaseTests(TestCase):
    def setUp(self):
        ...
        self.metadata_map = pd.DataFrame.from_dict(
            metadata_map, orient='index'
        )

        self.prop_expl = np.array(
            [
                25.6216900347,
                15.7715955926,
                14.1215046787,
        ...
        gr_wo_msg = GroupResults(
            'Foo',
            np.array([-2.6750, -0.2510, -2.8322, 0.0]),
            -1.4398,
            {'mean': -1.4398, 'std': 1.3184},
            None,
        )
        gr_w_msg = GroupResults(
            'Bar',
            np.array([9.6823, 2.9511, 5.2434]),
            5.9589,
            {'mean': 5.9589, 'std': 2.7942},
            'Cannot calculate the first difference '
            'with a window of size (3).',
        )
        self.groups = [gr_wo_msg, gr_w_msg]

        cr_no_data = CategoryResults(
            'foo',
            None,
            None,
        ...
        control_group = GroupResults(
            'Control',
            np.array([2.3694, 3.3716, 5.4452, 4.5704, 4.4972]),
            4.0508,
            {'avg': 4.0508},
            None,
        )
        fast_group = GroupResults(
            'Fast',
            np.array([7.2220, 4.2726, 1.1169, 4.0271]),
            4.1596,
            {'avg': 4.1596},
            None,
        )
        treatment = CategoryResults(
            'Treatment', 0.9331, [control_group, fast_group], None
        )
        vr_real = GradientANOVAResults(
            'avg', False, [description, weight, dob, treatment]
        )
    ...
    def assert_category_results_almost_equal(self, obs, exp):
        ...
        if exp.probability is None:
            ...
            for o, e in zip(
                sorted(obs.groups, key=attrgetter('name')),
    ...
    def assert_gradientANOVA_results_almost_equal(self, obs, exp):
        ...
        for o, e in zip(
            sorted(obs.categories, key=attrgetter('category')),
...
class GradientTests(BaseTests):
    def test_weight_by_vector(self):
        ...
        trajectory = pd.DataFrame.from_dict(
            {
                's1': np.array([1]),
                's2': np.array([2]),
                's3': np.array([3]),
        ...
        trajectory.sort_values(by=0, inplace=True)
        w_vector = pd.Series(
            np.array([25, 30, 35, 40, 45]), ['s2', 's3', 's4', 's5', 's6']
        ).astype(np.float64)
        exp = pd.DataFrame.from_dict(
            {
                's2': np.array([2]),
        ...
        trajectory = pd.DataFrame.from_dict(
            {
                'PC.356': np.array([5.65948525, 1.37977545, -4.9706303]),
                'PC.481': np.array([0.79151484, -0.70387996, 1.89223152]),
                'PC.355': np.array([6.05869624, 3.44821245, -0.42595788]),
                'PC.593': np.array([5.18731945, -1.81714206, 4.26216485]),
                'PC.354': np.array([7.07588529, -0.53917873, 0.89389158]),
            },
            orient='index',
        )
        w_vector = pd.Series(
            np.array([50, 52, 55, 57, 60]), sample_ids
        ).astype(np.float64)
        exp = pd.DataFrame.from_dict(
            {
                'PC.356': np.array([5.65948525, 1.37977545, -4.9706303]),
                'PC.481': np.array([0.98939355, -0.87984995, 2.3652894]),
                'PC.355': np.array([5.04891353, 2.87351038, -0.3549649]),
                'PC.593': np.array([6.48414931, -2.27142757, 5.32770606]),
                'PC.354': np.array([5.89657108, -0.44931561, 0.74490965]),
            },
            orient='index',
        )
        obs = _weight_by_vector(
            trajectory.loc[sample_ids], w_vector[sample_ids]
        )
        assert_data_frame_almost_equal(obs.sort_index(), exp.sort_index())

    def test_weight_by_vector_single_element(self):
        trajectory = pd.DataFrame.from_dict(
            {'s1': np.array([42])}, orient='index'
        )
    ...
    def test_ANOVA_trajectories(self):
        ...
        group = GroupResults(
            ...
            4.05080566264,
            {'avg': 4.0508056626409275},
            None,
        )
        obs = _ANOVA_trajectories('Foo', [group])
        exp = CategoryResults(
            'Foo', None, None, 'Only one value in the group.'
        )
        ...
        gr1 = GroupResults(
            'Foo',
            np.array([-0.219044992, 0.079674486, 0.09233683]),
            -0.015677892,
            {'avg': -0.015677892},
            None,
        )
        gr2 = GroupResults(
            'Bar',
            np.array([-0.042258081, 0.000204041, 0.024837603]),
            -0.0732878716,
            {'avg': -0.0732878716},
            None,
        )
...
class GradientANOVATests(BaseTests):
    def test_init(self):
        ...
        assert_data_frame_almost_equal(
            bv._coords.loc[self.coords_3axes.index], self.coords_3axes
        )
        exp_prop_expl = np.array([25.6216900347, 15.7715955926, 14.1215046787])
        npt.assert_equal(bv._prop_expl, exp_prop_expl)
        assert_data_frame_almost_equal(
            bv._metadata_map.loc[self.metadata_map.index],  # noqa
            self.metadata_map,
        )
        ...
        pdt.assert_series_equal(
            bv._weighting_vector.loc[exp_weighting_vector.index],  # noqa
    ...
    def test_init_error(self):
        ...
        with self.assertRaises(ValueError):
            GradientANOVA(
                self.coords, self.prop_expl, self.metadata_map, weighted=True
            )
        ...
        with self.assertRaises(ValueError):
            GradientANOVA(
                self.coords, self.prop_expl, self.metadata_map, axes=10
            )

        # Raises ValueError if axes < 0
        with self.assertRaises(ValueError):
            GradientANOVA(
                self.coords, self.prop_expl, self.metadata_map, axes=-1
            )
    ...
    def test_normalize_samples(self):
        ...
        subset_metadata_map = pd.DataFrame.from_dict(
            metadata_map, orient='index'
        )
        ...
        assert_data_frame_almost_equal(
            bv._coords.sort_index(), subset_coords.sort_index()
        )
        assert_data_frame_almost_equal(
            bv._metadata_map.sort_index(), subset_metadata_map.sort_index()
        )
        ...
        assert_data_frame_almost_equal(
            bv._coords.sort_index(), exp_coords.sort_index()
        )
        ...
        assert_data_frame_almost_equal(
            bv._metadata_map.sort_index(), exp_metadata_map.sort_index()
        )
    ...
    def test_make_groups(self):
        ...
        exp_groups = {
            'Treatment': {
                'Control': ['PC.354', 'PC.355', 'PC.356', 'PC.481', 'PC.593'],
                'Fast': ['PC.607', 'PC.634', 'PC.635', 'PC.636'],
            },
            'DOB': {
                '20061218': ['PC.354', 'PC.355'],
                '20061126': ['PC.356'],
                '20070314': ['PC.481'],
                '20071210': ['PC.593'],
                '20071112': ['PC.607'],
    ...
    def test_make_groups_natural_sorting(self):
        ...
        exp_groups = {
            'Col1': {'foo': ['a-10', 'a2', 'a11.0'], 'bar': ['a1', 'a10']},
            'Col2': {
                '1.0': ['a2'],
                '-42.0': ['a1'],
                '2e-5': ['a11.0'],
        ...
        exp_groups = {
            'Col1': {'foo': ['a11.0', 'a2', 'a-10'], 'bar': ['a1', 'a10']}
        }
...
class AverageGradientANOVATests(BaseTests):
    def test_get_trajectories_all(self):
        ...
        av = AverageGradientANOVA(
            self.coords, self.prop_expl, self.metadata_map
        )
        obs = av.get_trajectories()

        exp_description = CategoryResults(
            'Description',
            None,
            None,
        ...
        exp_fast_group = GroupResults(
            ...
            4.15968417703,
            {'avg': 4.1596841770278292},
            None,
        )
        exp_treatment = CategoryResults(
            'Treatment', 0.93311555, [exp_control_group, exp_fast_group], None
        )
        exp = GradientANOVAResults(
            'avg', False, [exp_description, exp_weight, exp_dob, exp_treatment]
        )
    ...
    def test_get_trajectories_weighted(self):
        ...
        exp_fast_group = GroupResults(
            'Fast',
            np.array([7.2187223286, 2.5522161282, 2.2349795861, 4.5278215248]),
            4.1334348919,
            {'avg': 4.1334348919},
            None,
        )
...
class TrajectoryGradientANOVATests(BaseTests):
    def test_get_trajectories(self):
        ...
        exp_fast_group = GroupResults(
            'Fast',
            np.array([11.2291654905, 3.9163741156, 4.4943507388]),
            6.5466301150,
            {'2-norm': 12.713431181},
            None,
        )
    ...
    def test_get_trajectories_weighted(self):
        ...
        exp_control_group = GroupResults(
            'Control',
            np.array([8.9850643421, 6.1617529749, 7.7989125908, 4.9666249268]),
            6.9780887086,
            {'2-norm': 14.2894710091},
            None,
        )
        exp_fast_group = GroupResults(
            'Fast',
            np.array([9.6823682852, 2.9511115209, 5.2434091953]),
            5.9589630005,
            {'2-norm': 11.3995901159},
            None,
        )
        exp_treatment = CategoryResults(
            'Treatment',
...
class FirstDifferenceGradientANOVATests(BaseTests):
    def test_get_trajectories(self):
        dv = FirstDifferenceGradientANOVA(
            self.coords,
            self.prop_expl,
            self.metadata_map,
            trajectory_categories=['Treatment'],
        ...
        exp_control_group = GroupResults(
            'Control',
            np.array([-1.5719245594, 0.0073716633, -3.0360721941]),
            -1.5335416967,
            {'mean': -1.5335416967, 'std': 1.2427771485},
            None,
        )
        exp_fast_group = GroupResults(
            'Fast',
            np.array([-7.3127913749, 0.5779766231]),
            -3.3674073758,
            {'mean': -3.3674073758, 'std': 3.9453839990},
            None,
        )
        exp_treatment = CategoryResults(
            'Treatment',
    ...
    def test_get_trajectories_weighted(self):
        ...
        exp_control_group = GroupResults(
            'Control',
            np.array([-2.8233113671, 1.6371596158, -2.8322876639]),
            -1.3394798050,
            {'mean': -1.3394798050, 'std': 2.1048051097},
            None,
        )
        exp_fast_group = GroupResults(
            'Fast',
            np.array([-6.7312567642, 2.2922976743]),
            -2.2194795449,
            {'mean': -2.2194795449, 'std': 4.5117772193},
            None,
        )
        exp_treatment = CategoryResults(
            'Treatment',
...
class WindowDifferenceGradientANOVATests(BaseTests):
    def test_get_trajectories(self):
        ...
        exp_control_group = GroupResults(
            'Control',
            np.array([-2.5790341819, -2.0166764661, -3.0360721941, 0.0]),
            -1.9079457105,
            {'mean': -1.9079457105, 'std': 1.1592139913},
            None,
        )
        exp_fast_group = GroupResults(
            'Fast',
            np.array([11.2291654905, 3.9163741156, 4.4943507388]),
            6.5466301150,
            {'mean': 6.5466301150, 'std': 3.3194494926},
            'Cannot calculate the first difference '
            'with a window of size (3).',
        )
        exp_treatment = CategoryResults(
    ...
    def test_get_trajectories_weighted(self):
        ...
        exp_control_group = GroupResults(
            'Control',
            np.array([-2.6759675112, -0.2510321601, -2.8322876639, 0.0]),
            -1.4398218338,
            {'mean': -1.4398218338, 'std': 1.31845790844},
            None,
        )
        exp_fast_group = GroupResults(
            'Fast',
            np.array([9.6823682852, 2.9511115209, 5.2434091953]),
            5.9589630005,
            {'mean': 5.9589630005, 'std': 2.7942163293},
            'Cannot calculate the first difference '
            'with a window of size (3).',
        )
        exp_treatment = CategoryResults(
