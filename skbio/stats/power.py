...
def paired_subsamples(meta, cat, control_cats, order=None, strict_match=True):
    ...
    ids = _draw_paired_samples(
        meta_pairs=meta_pairs, index=index, num_samps=min_obs
    )
...
def _compare_distributions(
    test, samples, num_p, counts=5, mode='ind', num_iter=100
):
    ...
    for idx in range(num_iter):
        if mode == 'matched':
            pos = np.random.choice(
                np.arange(0, samp_lens[0]), counts[0], replace=False
            )
...
def _check_subsample_power_inputs(
    ...
    ratio_counts = np.array(
        [id_counts[i] / ratio[i] for i in range(num_groups)]
    )
    ...
    sample_counts = np.arange(
        min_counts, min(max_counts, largest), counts_interval
    )
...
def _draw_paired_samples(meta_pairs, index, num_samps):
    ...
    set_pos = np.random.choice(index, int(num_samps), replace=False).astype(
        int
    )
    ...
    for set_ in set_list:
        num_ = counter[set_]
        r2 = [
            np.random.choice(col, num_, replace=False)
            for col in meta_pairs[set_]
        ]
        subs.append(r2)
