...
def _ANOVA_trajectories(category, res_by_group):
    ...
    if len(res_by_group) == 1:
        return CategoryResults(
            category, None, None, 'Only one value in the group.'
        )
...
class GroupResults:
    ...
    def to_files(self, out_f, raw_f):
        ...
        """
        out_f.write(
            'For group "%s", the group means is: %f\n' % (self.name, self.mean)
        )
...
class GradientANOVA:
    ...
    def __init__(
        ...
        if not trajectory_categories:
            ...
            for category in trajectory_categories:
                if category not in metadata_map:
                    raise ValueError(
                        'Category %s not present in metadata.' % category
                    )
        ...
        if weighted:
            ...
            try:
                self._weighting_vector = self._metadata_map[
                    sort_category
                ].astype(np.float64)
            except ValueError:
    ...
    def _normalize_samples(self):
        ...
        if not sample_ids:
            raise ValueError(
                'Coordinates and metadata map had no samples ' 'in common'
            )
    ...
    def _get_group_trajectories(self, group_name, sids):
        ...
        return self._compute_trajectories_results(
            group_name, trajectories.loc[sids]
        )
    ...
    def _compute_trajectories_results(self, group_name, trajectories):
        ...
        raise NotImplementedError(
            'No algorithm is implemented on the base ' 'class.'
        )
...
class AverageGradientANOVA(GradientANOVA):
    ...
    def _compute_trajectories_results(self, group_name, trajectories):
        ...
        self._message_buffer = []
        return GroupResults(
            group_name, trajectory, np.mean(trajectory), calc, msg
        )
...
class FirstDifferenceGradientANOVA(GradientANOVA):
    ...
    def _compute_trajectories_results(self, group_name, trajectories):
        ...
        if len(trajectories) == 1:
            ...
            trajectory = np.array(
                [np.linalg.norm(trajectories[1] - trajectories[0])]
            )
            calc = {'mean': trajectory[0], 'std': 0}
        else:
            vec_norm = np.array(
        ...
        if len(trajectories) == 1:
            ...
            if len(vec_norm) <= self._window_size:
                ...
                for idx in range(0, len(vec_norm) - self._window_size):
                    ...
                    element = np.mean(
                        vec_norm[(idx + 1) : (idx + 1 + self._window_size)],
                        axis=0,
                    )
