...
class MantelTestData(TestCase):
    def setUp(self):
        ...
        # on the end.
        self.minx_dm_extra = DistanceMatrix(
            [[0, 1, 2, 7], [1, 0, 3, 2], [2, 3, 0, 4], [7, 2, 4, 0]],
            ['0', '1', '2', 'foo'],
        )
        self.minz_dm_extra = DistanceMatrix(
            [
                [0, 0.5, 0.25, 3],
                [0.5, 0, 0.1, 24],
                [0.25, 0.1, 0, 5],
...
class InternalMantelTests(MantelTestData):
    ...
    def test_perm_pearsonr3(self):
        ...
        perm_order = np.asarray(
            [[2, 1, 0], [2, 0, 1], [0, 2, 1], [2, 0, 1]], dtype=int
        )
        xmean = 2.0
        ...
        for i in range(len(perm_order)):
            exp_res = self._compute_perf_one(
    ...
    def test_pearsonr_full(self):
        ...
        np.random.seed(0)
        orig_stat_fast, comp_stat, permuted_stats_fast = _mantel_stats_pearson(
            x, y, num_perms
        )
        ...
        perm_gen = (
            pearsonr(x.permute(condensed=True), y_flat)[0]
            for _ in range(num_perms)
        )
        permuted_stats = np.fromiter(perm_gen, float, count=num_perms)
    ...
    def test_spearmanr_full(self):
        ...
        perm_gen = (
            spearmanr(x.permute(condensed=True), y_flat)[0]
            for _ in range(num_perms)
        )
        permuted_stats = np.fromiter(perm_gen, float, count=num_perms)
...
class MantelTests(MantelTestData):
    ...
    def assert_mantel_almost_equal(self, left, right):
        ...
        npt.assert_almost_equal(left[1] + 0.5, right[1] + 0.5, decimal=2)
    ...
    def test_distance_matrix_instances_with_lookup(self):
        ...
        obs = mantel(
            self.minx_dm, self.miny_dm, alternative='less', lookup=lookup
        )
        self.assert_mantel_almost_equal(obs, [self.exp_x_vs_y, 0.843, 3])
    ...
    def test_one_sided_less(self):
        ...
        for method in self.methods:
            obs = mantel(
                self.minx, self.minx, method=method, alternative='less'
            )
    ...
    def test_two_sided(self):
        ...
        obs = mantel(
            self.minx, self.minx, method='spearman', alternative='two-sided'
        )
        self.assert_mantel_almost_equal(obs, [1.0, 0.328, 3])

        obs = mantel(
            self.minx, self.miny, method='spearman', alternative='two-sided'
        )
        self.assert_mantel_almost_equal(obs, [0.5, 1.0, 3])

        obs = mantel(
            self.minx, self.minz, method='spearman', alternative='two-sided'
        )
    ...
    def test_vegan_example(self):
        ...
        # pearson
        obs = mantel(
            self.veg_dm_vegan, self.env_dm_vegan, alternative='greater'
        )
    ...
    def test_no_variation_pearson(self):
        for alt in self.alternatives:
            ...
            # distances
            obs = mantel(
                self.miny, self.no_variation, method='pearson', alternative=alt
            )
            npt.assert_equal(obs, (np.nan, np.nan, 3))

            obs = mantel(
                self.no_variation, self.miny, method='pearson', alternative=alt
            )
            npt.assert_equal(obs, (np.nan, np.nan, 3))
...
class PairwiseMantelTests(MantelTestData):
    ...
    def assert_pwmantel_almost_equal(self, left, right):
        ...
        npt.assert_almost_equal(
            left.values[:, 1] + 0.5, right.values[:, 1] + 0.5, decimal=2
        )
    ...
    def test_minimal_compatible_input(self):
        ...
        # input as array_like
        obs = pwmantel(
            (self.minx, self.miny, self.minz), alternative='greater'
        )
        assert_data_frame_almost_equal(obs, self.exp_results_minimal)
    ...
    def test_minimal_compatible_input_with_labels(self):
        ...
        assert_data_frame_almost_equal(
            obs, self.exp_results_minimal_with_labels
        )

    def test_duplicate_dms(self):
        obs = pwmantel(
            (self.minx_dm, self.minx_dm, self.minx_dm), alternative='less'
        )
        assert_data_frame_almost_equal(obs, self.exp_results_duplicate_dms)

    def test_na_p_value(self):
        obs = pwmantel(
            (self.miny_dm, self.minx_dm), method='spearman', permutations=0
        )
    ...
    def test_id_lookup(self):
        ...
        obs = pwmantel(
            (x, y, z), alternative='greater', strict=False, lookup=lookup
        )
        assert_data_frame_almost_equal(
            obs, self.exp_results_reordered_distance_matrices
        )
