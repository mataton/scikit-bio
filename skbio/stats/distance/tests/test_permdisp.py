...
class testPERMDISP(TestCase):
    def setUp(self):
        ...
        self.assert_series_equal = partial(
            assert_series_equal, check_index_type=True, check_series_type=True
        )

    def test_centroids_eq_groups(self):
        exp = [
            [1.2886811963240687, 1.890538910062923, 1.490527658097728],
    ...
    def test_centroids_uneq_groups(self):
        ...
        """
        exp = [
            [2.5847022428144935, 2.285624595858895, 1.7022431146340287],
    ...
    def test_centroid_normal(self):
        exp = pd.Series(
            ...
            data=['PERMDISP', 'F-value', 9, 2, 0.244501519876, 0.63, 99],
        ...
        np.random.seed(0)
        obs = permdisp(
            self.unifrac_dm, grouping, test='centroid', permutations=99
        )
    ...
    def test_median_normal(self):
        exp = pd.Series(
            index=self.exp_index,
            data=['PERMDISP', 'F-value', 9, 2, 0.139475441876, 0.61, 99],
            name='PERMDISP results',
        )

        np.random.seed(0)
        obs = permdisp(
            self.unifrac_dm, self.unif_grouping, test='median', permutations=99
        )
        ...
        obs2 = permdisp(po, self.unif_grouping, test='median', permutations=99)
    ...
    def test_median_fsvd(self):
        exp = pd.Series(
            index=self.exp_index,
            data=['PERMDISP', 'F-value', 9, 2, 0.04078077215673714, 0.8, 99],
            name='PERMDISP results',
        )
        ...
        obs = permdisp(po, self.unif_grouping, test='median', permutations=99)
    ...
    def test_confirm_betadispr_results(self):
        ...
        obs_med_mp = permdisp(mp_dm, mp_mf, column='BodySite')
    ...
    def test_call_via_series(self):
        ...
        grouping = pd.read_csv(
            get_data_path('frameSeries_grouping.tsv'), sep='\t', index_col=0
        )
