...
class TestPERMANOVA(TestCase):
    ...
    def test_call_via_series(self):
        ...
        grp_frame = _preprocess_input_sng(
            ...
            dm.shape[0],
            grouping,
            'tumor',  # grouping as a pd.DataFrame
        )[-1]
        grp_series = _preprocess_input_sng(
            dm.ids,
            dm.shape[0],
            grouping['tumor'],
            column=None,  # grouping as a pd.Series, note
        ...
        with self.assertRaises(ValueError):
            _preprocess_input_sng(
                dm.ids, dm.shape[0], grouping['tumor'], 'foo'
            )
