...
class TabularMSA(MetadataMixin, PositionalMetadataMixin, SkbioObject):
    ...
    def __init__(
        ...
        if isinstance(sequences, TabularMSA):
            ...
            if (
                positional_metadata is None
                and sequences.has_positional_metadata()
            ):
    ...
    def __deepcopy__(self, memo):
        ...
        msa_copy._positional_metadata = PositionalMetadataMixin._deepcopy_(
            self, memo
        )
    ...
    def consensus(self):
        ...
        return dtype(
            ''.join(consensus), positional_metadata=positional_metadata
        )
    ...
    def conservation(
        ...
        if degenerate_mode not in {'nan', 'error'}:
            raise ValueError(
                'Unknown degenerate_mode provided: %s' % degenerate_mode
            )
        ...
        for p in self.iter_positions(ignore_metadata=True):
            ...
            if pos_seq.has_gaps():
                if gap_mode == 'nan':
                    ...
                    pos_seq = pos_seq.replace(
                        pos_seq.gaps(), pos_seq.default_gap_char
                    )
    ...
    def append(self, sequence, minter=None, index=None, reset_index=False):
        ...
        if index is not None:
            index = [index]
        self.extend(
            [sequence], minter=minter, index=index, reset_index=reset_index
        )
    ...
    def extend(self, sequences, minter=None, index=None, reset_index=False):
        ...
        """
        if (
            sum([minter is not None, index is not None, bool(reset_index)])
            != 1
        ):
            raise ValueError(
    ...
    def to_dict(self):
        ...
        if self.index.is_unique:
            ...
            raise ValueError(
                'Cannot convert to dict. Index labels are not' ' unique.'
            )
