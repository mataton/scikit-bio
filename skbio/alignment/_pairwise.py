...
def global_pairwise_align_nucleotide(
    ...
    for seq in seq1, seq2:
        ...
        if isinstance(seq, TabularMSA) and not issubclass(
            seq.dtype, (DNA, RNA)
        ):
            raise TypeError(
...
def make_identity_substitution_matrix(
    match_score, mismatch_score, alphabet='ACGTU'
):
...
def _compute_score_and_traceback_matrices(
    ...
    for aln2_pos, aln2_chars in enumerate(
        aln2.iter_positions(ignore_metadata=True), 1
    ):
...
def _traceback(
    traceback_matrix, score_matrix, aln1, aln2, start_row, start_col
):
    ...
    for i, (aligned_seq, original) in enumerate(zip(aligned_seqs1, aln1)):
        ...
        aligned_seqs1[i] = constructor(
            aligned_seq, metadata=metadata, validate=False
        )
