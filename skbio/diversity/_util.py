...
def _validate_counts_matrix(counts, ids=None, suppress_cast=False):
    ...
    if isinstance(counts, pd.core.frame.DataFrame):
        ...
        if len(counts) == 0 or not isinstance(
            counts[0], collections.abc.Iterable
        ):
            counts = [counts]
        counts = np.asarray(counts)
        if counts.ndim > 2:
...
def _validate_otu_ids_and_tree(counts, otu_ids, tree, rooted=True):
    ...
    if missing_tip_names != set():
        ...
        raise MissingNodeError(
            ...
            'tip names (n=%d): %s'
            % (n_missing_tip_names, ' '.join(missing_tip_names))
        )
...
def _get_phylogenetic_kwargs(counts, **kwargs):
    ...
    try:
        ...
        raise ValueError(
            '``tree`` is required for phylogenetic diversity ' 'metrics.'
        )
