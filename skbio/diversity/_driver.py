...
def alpha_diversity(metric, counts, ids=None, validate=True, **kwargs):
    ...
    if metric == 'faith_pd':
        ...
        metric = functools.partial(
            _faith_pd, branch_lengths=branch_lengths, **kwargs
        )
        ...
        if 'weight' not in kwargs:
            kwargs['weight'] = False
        metric = functools.partial(
            _phydiv, branch_lengths=branch_lengths, **kwargs
        )
...
def partial_beta_diversity(
    metric, counts, ids, id_pairs, validate=True, **kwargs
):
