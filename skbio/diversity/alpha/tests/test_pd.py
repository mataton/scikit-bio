...
class FaithPDTests(TestCase):
    ...
    def test_faith_pd_none_observed(self):
        actual = faith_pd(
            np.array([], dtype=int), np.array([], dtype=int), self.t1
        )
        expected = 0.0
        self.assertAlmostEqual(actual, expected)
    ...
    def test_faith_pd_all_observed(self):
        ...
        expected = sum(
            n.length for n in self.t1.traverse() if n.length is not None
        )
        self.assertAlmostEqual(actual, expected)
    ...
    def test_faith_pd_qiime_tiny_test(self):
        ...
        self.q_table = pd.read_csv(
            tt_table_fp, sep='\t', skiprows=1, index_col=0
        )
        ...
        for sid in self.q_table.columns:
            actual = faith_pd(
                self.q_table[sid], otu_ids=self.q_table.index, tree=self.q_tree
            )
    ...
    def test_faith_pd_invalid_input(self):
        ...
        t = TreeNode.read(
            StringIO('((OTU1:0.1, OTU2:0.2):0.3, OTU3:0.5,' 'OTU4:0.7);')
        )
        counts = [1, 2, 3]
        otu_ids = ['OTU1', 'OTU2', 'OTU3']
...
class PhyDivTests(TestCase):
        ...
        self.data = np.array(
            [
                ...
                [
                    ...
                    5,
                    6,
                ],  # non-basal, monophyletic group (c, d, e, f)
                [1, 2, 3, 4, 0, 0],
            ]
        )  # basal, non-monophyletic group (a, b, c, d)
    ...
    def test_phydiv_unrooted_weighted(self):
        ...
        obs = phydiv(
            self.data[0], self.taxa, self.tree, rooted=False, weight=True
        )
        exp = 0.6
        self.assertAlmostEqual(obs, exp)
        ...
        obs = phydiv(
            self.data[1], self.taxa, self.tree, rooted=False, weight=True
        )
        exp = 0.25714286
        self.assertAlmostEqual(obs, exp)
        ...
        #   min(3 + 4, 5 + 6)) / (3 + 4 + 5 + 6)
        obs = phydiv(
            self.data[3], self.taxa, self.tree, rooted=False, weight=True
        )
        exp = 1.21111111
        self.assertAlmostEqual(obs, exp)
        ...
        #   min(4, 1 + 2 + 3)) / (1 + 2 + 3 + 4)
        obs = phydiv(
            self.data[4], self.taxa, self.tree, rooted=False, weight=True
        )
        exp = 1.4
        self.assertAlmostEqual(obs, exp)
