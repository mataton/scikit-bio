...
fuzzy=[(False, False), (False, False)], metadata={'name': 'genA'})
    ...
    def _bounds_fuzzy_setter(self, bounds=None, fuzzy=None):
        ...
        if bounds is None:
            ...
            if fuzzy is None:
                raise ValueError(
                    'Cannot give `None` to both `bounds` ' 'and `fuzzy`.'
                )
    ...
    def fuzzy(self):
        ...
        if self.dropped:
            raise RuntimeError(
                'Cannot change fuzzy on dropped ' 'Interval object.'
            )
    ...
    def metadata(self, value):
        if self.dropped:
            raise RuntimeError(
                'Cannot change metadata on dropped ' 'Interval object.'
            )
    ...
    def metadata(self):
        ...
        if self.dropped:
            raise RuntimeError(
                'Cannot change metadata to dropped ' 'Interval object.'
            )
        self._metadata = {}
...
fuzzy=[(True, True)], metadata={'gene': 'sagB'})
        ...
        for i, im in enumerate(interval_metadata):
            for intvl in im._intervals:
                ...
                if i != 0:
                    bounds = [
                        (start + length, end + length) for start, end in bounds
                    ]
    ...
    def __eq__(self, other):
        ...
        if (
            ...
            self_intervals = sorted(
                self._intervals, key=operator.attrgetter('bounds')
            )
...
def _assert_valid_bound(bound):
    if isinstance(bound, tuple):
        ...
        if (
            ...
            raise ValueError(
                '`start` (%r) cannot be a larger int '
...
def _assert_valid_fuzzy(fuzzy):
    if isinstance(fuzzy, tuple):
        try:
            ...
            raise ValueError(
                'A `fuzzy` must be a tuple of exactly ' 'two, not {!r}'.format(
                    fuzzy
                )
            )
