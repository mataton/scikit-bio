...
from skbio.metadata._interval import _assert_valid_bound, _assert_valid_fuzzy
...
class TestInterval(unittest.TestCase, ReallyEqualMixin):
    ...
    def test_init(self):
        ...
        self.assertDictEqual(
            f.metadata, {'name': 'sagA', 'function': 'transport'}
        )
    ...
    def test_repr(self):
        ...
        f.drop()
        exp = (
            r'Interval\(dropped=True, bounds=\[\(1, 2\)\],'
            r" fuzzy=\[\(False, False\)\], metadata={'name': 'sagA'}\)"
        )
        obs = repr(f)
        self.assertRegex(obs, exp)
...
class TestIntervalMetadata(unittest.TestCase, ReallyEqualMixin):
    ...
    def test_deepcopy(self):
        ...
        self.assertEqual(
            i2.metadata, {'gene': 'sagB', 'bound': 0, 'spam': [0, 1]}
        )
        self.assertEqual(
            i1.metadata, {'gene': 'sagB', 'bound': 0, 'spam': [0]}
        )
    ...
    def test_upper_bound_is_none(self):
        ...
        with self.assertRaisesRegex(TypeError, r'upper bound is `None`'):
            im._reverse()
        with self.assertRaisesRegex(TypeError, r'upper bound is `None`'):
    ...
    def test_concat(self):
        ...
        exp.add(
            bounds=[(3, 6)], fuzzy=[(True, False)], metadata={'gene': 'sagA'}
        )
        exp.add(bounds=[(5, 7)], metadata={'gene': 'sagB'})
        exp.add(
            bounds=[(8, 12)], fuzzy=[(False, True)], metadata={'gene': 'sagC'}
        )
        self.assertEqual(obs, exp)
    ...
    def test_merge_unequal_upper_bounds(self):
        ...
        for im in [self.im_empty, self.im_1]:
            with self.assertRaisesRegex(
                ValueError, r'not equal \(%d != %d\)' % (self.upper_bound, n)
            ):
                im.merge(im1)
    ...
    def test_sort(self):
        interval = Interval(
            self.im_2, [(1, 2), (3, 8)], metadata={'gene': 'sagA', 'bound': 0}
        )
        ...
        self.assertEqual(
            self.im_2._intervals, [self.im_2_2, self.im_2_1, interval]
        )

        self.im_2.sort()
        self.assertEqual(im, self.im_2)
        self.assertEqual(
            self.im_2._intervals, [interval, self.im_2_1, self.im_2_2]
        )
    ...
    def test_add_gt_upper_bound(self):
        with self.assertRaises(ValueError):
            self.im_empty.add(
                bounds=[(1, 2), (4, self.upper_bound + 1)],
                metadata={'gene': 'sagA', 'bound': 0},
            )
    ...
    def test_add_eq_start_end_bound(self):
        for i in 0, 1, self.upper_bound:
            # test that no exception is raised
            self.im_empty.add(
                bounds=[(i, i)], metadata={'gene': 'sagA', 'bound': 0}
            )
    ...
    def test_query_interval_upper_bound(self):
        intervals = list(
            self.im_2._query_interval((self.upper_bound - 1, self.upper_bound))
        )
    ...
    def test_eq_ne(self):
        ...
        im1.add(
            metadata={'gene': 'sagA', 'bound': '0'}, bounds=[(0, 2), (4, 7)]
        )
        ...
        im3.add(
            metadata={'gene': 'sagA', 'bound': '3'}, bounds=[(0, 2), (4, 7)]
        )
