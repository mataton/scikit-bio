...
class TreeNode(SkbioObject):
    ...
    _exclude_from_copy = set(
        ['parent', 'children', '_tip_cache', '_non_tip_cache']
    )
    ...
    def copy(self):
        ...
        while nodes_stack:
            ...
            if unvisited_children:
                ...
                nodes_stack.append(
                    [new_child, old_child, len(old_child.children)]
                )
    ...
    def unrooted_copy(self, parent=None):
        ...
        result = self.__class__(
            name=edgename, children=children, length=length
        )
    ...
    def root_at(self, node):
        ...
        if not node.children:
            raise TreeError(
                "Can't use a tip (%s) as the root" % repr(node.name)
            )
        ...
        for n in self.postorder():
            if n.is_tip():
                ...
                if len(n.children) == 1:
                    ...
                    tip_info = [
                        (max(c.MaxDistTips, key=maxkey), c) for c in n.children
                    ]
    ...
    def tip_tip_distances(self, endpoints=None):
        ...
        if endpoints is None:
            ...
            for n in tip_order:
                if not n.is_tip():
                    raise ValueError(
                        "Node with name '%s' is not a tip." % n.name
                    )
    ...
    def index_tree(self):
        ...
        for n in self.postorder():
            for c in n.children:
                ...
                if c:
                    ...
                    child_index.append(
                        (c.id, c.children[0].id, c.children[-1].id)
                    )
        ...
        if self.children:
            child_index.append(
                (self.id, self.children[0].id, self.children[-1].id)
            )
    ...
    def descending_branch_length(self, tip_subset=None):
        ...
        if tip_subset is not None:
            ...
            if not set(tip_subset).issubset(all_tips):
                raise ValueError(
                    "tip_subset contains ids that aren't tip " 'names.'
                )
            ...
            for tip in tip_subset:
                ...
                while curr is not lca:
                    ancestors[curr.id] = (
                        curr.length if curr.length is not None else 0.0
                    )
                    curr = curr.parent
