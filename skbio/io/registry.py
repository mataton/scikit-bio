...
class IORegistry:
    ...
    def sniff(self, file, **kwargs):
        ...
        # next sniffer)
        with _resolve_file(file, mode='r', **kwargs) as (
            fh,
            _,
            is_binary_file,
        ):
            ...
            if is_binary_file and kwargs.get('encoding', 'binary') == 'binary':
                matches = self._find_matches(
                    fh, self._binary_formats, **kwargs
                )
            ...
            if kwargs.get('encoding', None) != 'binary':
                ...
                raise ValueError(
                    'Cannot decode text source (%r) as binary.' % file
                )
        ...
        if len(matches) > 1:
            ...
            raise UnrecognizedFormatError(
                'Could not detect the format of %r' % file
            )
    ...
    def _init_reader(self, file, fmt, into, verify, kwargs, io_kwargs):
        ...
        if reader is None:
            possible_intos = [
                r.__name__ for r in self._get_possible_readers(fmt)
            ]
            message = ''
            if possible_intos:
    ...
    def _validate_encoding(self, encoding):
        if encoding != self._encoding:
            if self._encoding == 'binary':
                raise ValueError(
                    "Encoding must be 'binary' for %r" % self.name
                )
            if encoding == 'binary':
                raise ValueError(
                    "Encoding must not be 'binary' for %r" % self.name
                )
...
def read(file, format=None, into=None, verify=True, **kwargs):
    return io_registry.read(
        file, format=format, into=into, verify=verify, **kwargs
    )
