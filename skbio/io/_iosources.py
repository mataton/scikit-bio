...
class WrappedTemporaryFileSource(IOSource):
    ...
    def can_read(self):
        return (
            isinstance(self.file, _WrappedTemporaryFile)
            and self.file.readable()
        )

    def can_write(self):
        return (
            isinstance(self.file, _WrappedTemporaryFile)
            and self.file.writable()
        )
...
class IterableSource(IOSource):
    ...
    def get_reader(self):
        return IterableStringReaderIO(
            self.repaired, newline=self.options['newline']
        )

    def get_writer(self):
        return IterableStringWriterIO(
            self.file, newline=self.options['newline']
        )
