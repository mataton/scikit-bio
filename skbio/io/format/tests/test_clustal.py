...
from skbio.io.format.clustal import (
    _clustal_to_tabular_msa,
    _tabular_msa_to_clustal,
    _clustal_sniffer,
    _is_clustal_seq_line,
...
class ClustalHelperTests(TestCase):
    def test_label_line_parser(self):
        self.assertEqual(
            _label_line_parser(StringIO('abc\tucag')),
            ({'abc': ['ucag']}, ['abc']),
        )
    ...
    def test_check_lengh(self):
        ...
        self.assertEqual(
            ...
            _check_length(
                {
                    'abc': ['adjfkadfjaksdlfadskfd', 'adjfkadfjaksdlfadskfda'],
                    'xyz': [
                        'adjfkadfjaksdlfadsksdf',
                        'adjfkadfjaksdlfadsksdf',
                    ],
            ...
            _check_length(
                {
                    ...
                    'xyz': ['adjfkadfjaksdlfadsksdf', 'adjfkadfjaksdlfadsksd'],
                },
                ['abc', 'xyz'],
            ),
        )
...
xyz             UGCUGCAUCA---------------- 33
        ...
        self.invalid_clustal_out = [
            ...
            StringIO('\n'.join(['hfsdjksdfhjsdf'])),
            StringIO(
                '\n'.join(['dshfjsdfhdfsj', 'dshfjsdfhdfsj', 'hfsdjksdfhjsdf'])
            ),
            StringIO('\n'.join(['dshfjsdfhdfsj', '\t', 'hfsdjksdfhjsdf'])),
...
*     ***"""
    ...
    def test_tabular_msa_to_clustal_with_empty_input(self):
        result = _clustal_to_tabular_msa(
            StringIO(), constructor=CustomSequence
        )
    ...
    def test_invalid_tabular_msa_to_clustal_and_clustal_to_tabular_msa(self):
        for invalid_out in self.invalid_clustal_out:
            with self.assertRaises(ClustalFormatError):
                dict(
                    _clustal_to_tabular_msa(
                        invalid_out, constructor=CustomSequence
                    )
                )
    ...
    def test_duplicate_labels(self):
        msa = TabularMSA(
            [CustomSequence('foo'), CustomSequence('bar')], index=['a', 'a']
        )
