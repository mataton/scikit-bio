...
from skbio.io.format.binary_dm import (
    _h5py_mat_to_skbio_mat,
    _skbio_mat_to_h5py_mat,
    _get_header,
    _parse_ids,
    _verify_dimensions,
    _bytes_decoder,
...
class BinaryMatrixTests(unittest.TestCase):
    def setUp(self):
        self.mat = np.array([[0, 0.1, 0.2], [0.1, 0, 0.3], [0.2, 0.3, 0]])
        self.ids = ['a', 'b', 'c']
        ...
        ids = self.badids.create_dataset(
            'order', shape=(2,), dtype=_vlen_dtype
        )
        ids[:] = ['a', 'b']
        ...
        ids = self.noheader.create_dataset(
            'order', shape=(3,), dtype=_vlen_dtype
        )
        ids[:] = self.ids
    ...
    def test_binary_dm_sniffer(self):
        self.assertEqual(
            (True, {}), _binary_dm_sniffer(open(self.basic_fname, 'rb'))
        )
        self.assertEqual(
            (False, {}), _binary_dm_sniffer(open(self.badids_fname, 'rb'))
        )
        self.assertEqual(
            (False, {}), _binary_dm_sniffer(open(self.noheader_fname, 'rb'))
        )

    def test_h5py_mat_to_skbio_mat(self):
        exp = DistanceMatrix(self.mat, self.ids)
        obs = _h5py_mat_to_skbio_mat(
            DistanceMatrix, h5py.File(self.basic_fname, 'r')
        )
    ...
    def test_get_header(self):
        self.assertEqual(
            _get_header(h5py.File(self.basic_fname, 'r')),
            {'format': b'BDSM', 'version': b'2020.06'},
        )
        self.assertEqual(
            _get_header(h5py.File(self.noheader_fname, 'r')), None
        )

    def test_parse_ids(self):
        tests = [
            (['a', 'b', 'c'], ['a', 'b', 'c']),
            ([b'a', b'b', b'\xc3\xa9\xc3\xb8asd'], ['a', 'b', 'éøasd']),
        ]
