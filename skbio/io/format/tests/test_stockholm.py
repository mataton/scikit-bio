...
from skbio.io.format.stockholm import (
    _stockholm_to_tabular_msa,
...
class TestStockholmSniffer(unittest.TestCase):
    def setUp(self):
        ...
        self.negatives = [
            get_data_path(e)
            for e in ['stockholm_missing_header', 'empty', 'whitespace_only']
        ]
...
class TestStockholmReader(unittest.TestCase):
    def test_stockholm_extensive(self):
        ...
        exp = TabularMSA(
            ...
            positional_metadata={
                'SS_cons': list(
                    'CCCCCHHHHHHHH' 'HHHHH..EEEEEE' 'EE....EEEEEEE' 'EEEH'
                )
            },
    ...
    def test_stockholm_rna(self):
        ...
        exp = TabularMSA(
            ...
            positional_metadata=({'AC_cons': list('GGGACUGGACAUCUAUUCAG')}),
    ...
    def test_stockholm_runon_gf(self):
        ...
        exp = TabularMSA(
            [DNA('ACTGGTTCAATG')],
            metadata={
                'CC': 'CBS domains are small intracellular'
    ...
    def test_stockholm_metadata_only(self):
        ...
        msa = _stockholm_to_tabular_msa(fp, constructor=DNA)
        exp = TabularMSA(
            [], metadata={'NM': 'Kestrel Gorlick', 'DT': 'February 5th, 2016'}
        )
        self.assertEqual(msa, exp)
    ...
    def test_stockholm_multiple_trees(self):
        ...
        exp = TabularMSA(
            [],
            metadata={
                'NH': {'tree1': 'ABCD', 'tree2': 'EFGH', 'tree3': 'IJKL'}
            },
        )
        self.assertEqual(msa, exp)
    ...
    def test_stockholm_to_msa_different_padding(self):
        ...
        exp = TabularMSA(
            ...
            metadata={
                'RN': [
                    OrderedDict(
                        [('RC', 'A Runon Comment Without ' 'Whitespace')]
                    ),
                    OrderedDict(
                        [('RC', 'A Runon Comment With ' 'Whitespace')]
                    ),
                ]
            },
        )
    ...
    def test_stockholm_handles_missing_reference_items(self):
        ...
        exp = TabularMSA(
            [],
            metadata={
                'RN': [OrderedDict([('RT', 'A Title'), ('RA', 'The Author')])]
            },
        )
        self.assertEqual(msa, exp)
    ...
    def test_stockholm_multiple_multi_line_trees(self):
        ...
        msa = _stockholm_to_tabular_msa(fp, constructor=DNA)
        exp = TabularMSA(
            [], metadata={'NH': {'tree1': 'ABCDEFGH', 'tree2': 'IJKLMNOP'}}
        )
        self.assertEqual(msa, exp)
    ...
    def test_stockholm_duplicate_tree_id_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, r'Tree.*tree1.*in file.'
        ):
    ...
    def test_stockholm_missing_reference_number_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, r"Expected 'RN'.*'RL' tag."
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_nonexistent_gr_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, r'GS or GR.*nonexistent ' 'sequence.*RL1355.'
        ):
    ...
    def test_nonexistent_gs_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, r'GS or GR.*nonexistent sequence.*AC14.'
        ):
            _stockholm_to_tabular_msa(fp, constructor=RNA)
    ...
    def test_duplicate_gr_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError,
            r'Found duplicate GR.*OS.*LFDR3.*supported' ' by the reader.',
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_duplicate_gc_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError,
            r'Found duplicate GC.*SS_cons.*supported ' 'by the reader.',
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_missing_header_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, r'File missing.*header'
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_missing_footer_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, r'Final line.*only "//".'
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_data_type_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, r"Unrecognized.*'#=GZ"
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_malformed_gf_line_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, r'Line contains 2.*must contain.*3.'
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_malformed_gs_line_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, r'Line contains 3.*must contain.*4.'
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_malformed_gr_line_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, r'Line contains 2.*must contain.*4.'
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_malformed_data_line_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError, r'Line contains 1.*must contain.*2.'
        ):
            _stockholm_to_tabular_msa(fp, constructor=DNA)
    ...
    def test_unsupported_constructor_error(self):
        fp = get_data_path('empty')
        with self.assertRaisesRegex(
            TypeError, r'`constructor`.*`GrammaredSequence`.'
        ):
...
class TestStockholmWriter(unittest.TestCase):
    def test_msa_to_stockholm_extensive(self):
        ...
        msa = TabularMSA(
            [
                DNA(
                    'GAGGCCATGCCCAGGTGAAG',
                    metadata=OrderedDict(
                        [('DT', 'February 1, 2016'), ('NM', 'Unknown')]
                    ),
                ),
                DNA('ACCTGAGCCACAGTAGAAGT'),
                DNA(
                    'CCCTTCGCTGGAAATGTATG',
                    metadata={'DT': 'Unknown'},
                    positional_metadata=OrderedDict(
                        [
                            ('AS', list('CCGAAAGT' 'CGTTCGA' 'AAATG')),
                            ('SS', list('GGCGAGTC' 'GTTCGAGC' 'TGG' 'C')),
                        ]
                    ),
                ),
            ],
            ...
            positional_metadata=OrderedDict(
                [
                    ('AS_cons', list('CGTTCGTTCTAAC' 'AATTCCA')),
                    ('SS_cons', list('GGCGCTACGACCT' 'ACGACCG')),
                ]
            ),
    ...
    def test_msa_to_stockholm_nonstring_values(self):
        ...
        msa = TabularMSA(
            [
                DNA(
                    'ACTG',
                    metadata=OrderedDict([(8, 123)]),
                    positional_metadata=OrderedDict([(1.0, [1, 2, 3, 4])]),
                )
            ],
    ...
    def test_unoriginal_index_error(self):
        msa = TabularMSA(
            [DNA('ATCGCCAGCT'), DNA('TTGTGCTGGC')], index=['seq1', 'seq1']
        )
        with self.assertRaisesRegex(
            StockholmFormatError, r'index labels must be unique.'
        ):
            fh = io.StringIO()
            _tabular_msa_to_stockholm(msa, fh)
    ...
    def test_gc_wrong_dataframe_item_length_error(self):
        ...
        msa = TabularMSA(
            [DNA('TCCTTGAACTACCCGA')],
            positional_metadata=pos_metadata_dataframe,
        )

        message = (
            r'Multiple sequence alignment positional metadata.*must '
    ...
    def test_rn_not_list_of_refs_error(self):
        ...
        with self.assertRaisesRegex(
            StockholmFormatError,
            r"Expected 'RN'.*list of reference" ".*got '1'",
        ):
            fh = io.StringIO()
            _tabular_msa_to_stockholm(msa, fh)
    ...
    def test_rn_data_not_in_dict_error(self):
        msa = TabularMSA(
            [],
            metadata={
                'RN': [OrderedDict([('RL', 'Flagstaff')]), 'Incorrect Item']
            },
        )
        with self.assertRaisesRegex(
            StockholmFormatError,
    ...
    def test_invalid_reference_tag_error(self):
        msa = TabularMSA(
            [],
            metadata={
                'RN': [OrderedDict([('RL', 'Flagstaff'), ('foo', 'bar')])]
            },
        )
        with self.assertRaisesRegex(
            StockholmFormatError,
