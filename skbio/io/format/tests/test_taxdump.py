...
class TestTaxdumpReader(unittest.TestCase):
    ...
    def test_custom_scheme(self):
        fs = StringIO(
            '\n'.join(map('\t|\t'.join, [('a', 'a'), ('b', 'a'), ('c', 'a')]))
        )
        obs = _taxdump_to_data_frame(fs, scheme={'self': str, 'parent': str})
        exp = pd.DataFrame(
            [['a', 'a'], ['b', 'a'], ['c', 'a']], columns=['self', 'parent']
        ).set_index('self')
        assert_data_frame_almost_equal(obs, exp)
    ...
    def test_invalid_scheme(self):
        ...
        self.assertEqual(
            str(ctx.exception), 'Invalid taxdump column scheme: "hello".'
        )
    ...
    def test_invalid_id(self):
        ...
        self.assertEqual(str(ctx.exception), 'Invalid taxdump file format.')
