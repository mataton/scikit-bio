...
from skbio.io.format.ordination import (
    _ordination_to_ordination_results,
    _ordination_results_to_ordination,
    _ordination_sniffer,
)
...
class OrdinationTestData(TestCase):
    def setUp(self):
        ...
        self.invalid_fps = map(
            lambda e: (get_data_path(e[0]), e[1], e[2]),
            [
                ...
                (
                    'ordination_error2',
                    r'Proportion explained header not found',
                    False,
                ),
                ...
                (
                    'ordination_error6',
                    r'Site constraints header not found',
                    True,
                ),
                ...
                (
                    'ordination_error11',
                    r'Site constraints ids and site ids',
                    True,
                ),
                ...
                (
                    'ordination_error18',
                    r'proportion explained.*eigvals: 8 != 9',
                    True,
                ),
                (
                    'ordination_error19',
                    r'coordinates.*species.*eigvals: 1 != 2',
                    True,
                ),
                (
                    'ordination_error20',
                    r'coordinates.*site.*eigvals: 1 != 2',
                    True,
                ),
                ...
                (
                    'ordination_error24',
                    r'end of file.*row 2.*Species section',
                    True,
                ),
            ],
        )
...
class OrdinationResultsReaderWriterTests(OrdinationTestData):
    def setUp(self):
        ...
        species = pd.DataFrame(
            [
                [0.408869425742, 0.0695518116298],
        ...
        species = pd.DataFrame(
            np.loadtxt(get_data_path('ordination_exp_Ordination_CCA_species')),
            index=species_ids,
            columns=axes_ids,
        )
        site = pd.DataFrame(
        ...
        species = pd.DataFrame(
            np.loadtxt(get_data_path('ordination_exp_Ordination_RDA_species')),
            index=species_ids,
            columns=axes_ids,
        )
        site = pd.DataFrame(
    ...
    def test_read_valid_files(self):
        for fp, obj in zip(self.valid_fps, self.ordination_results_objs):
            ...
            assert_ordination_results_equal(
                obs, obj, ignore_method_names=True, ignore_axis_labels=True
            )

    def test_read_invalid_files(self):
        for invalid_fp, error_msg_regexp, _ in self.invalid_fps:
            with self.assertRaisesRegex(
                OrdinationFormatError, error_msg_regexp
            ):
...
class SnifferTests(OrdinationTestData):
    ...
    def test_matches_and_nonmatches(self):
        ...
        for fp, _, expected_sniffer_match in self.invalid_fps:
            self.assertEqual(
                _ordination_sniffer(fp), (expected_sniffer_match, {})
            )
