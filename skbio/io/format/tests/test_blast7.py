...
class TestBlast7Reader(unittest.TestCase):
    ...
    def test_custom_valid_single_line(self):
        ...
        df = _blast7_to_data_frame(fp)
        exp = pd.DataFrame(
            [['query1', 100.00, 100.00, 8.0, 0.0, 16.9, 8.0, 'PAAWWWWW']],
            columns=[
                'qseqid',
                'ppos',
                'pident',
    ...
    def test_custom_valid_multi_line(self):
        ...
        exp = pd.DataFrame(
            [
                [1.0, 8.0, 3.0, 10.0, 8.0, 0.0, 1.0, 'query1', 'subject2'],
                [2.0, 5.0, 2.0, 15.0, 8.0, 0.0, 2.0, 'query1', 'subject2'],
                [1.0, 6.0, 2.0, 12.0, 8.0, 0.0, 1.0, 'query1', 'subject2'],
            ],
            columns=[
                'qstart',
                'qend',
    ...
    def test_custom_valid_mixed_nans(self):
        ...
        exp = pd.DataFrame(
            [
                [0.0, np.nan, 8.0, 13.0, 1.0, 1.0, np.nan, 'subject2'],
                [np.nan, 0.0, 8.0, np.nan, 1.0, 1.0, 'query1', np.nan],
            ],
            columns=[
                'qgi',
                'sgi',
    ...
    def test_no_data_error(self):
        ...
        with self.assertRaisesRegex(BLAST7FormatError, r'File contains no'):
            _blast7_to_data_frame(fp)
    ...
    def test_wrong_amount_of_columns_error(self):
        ...
        with self.assertRaisesRegex(
            BLAST7FormatError, r'Number of fields.*\(2\)'
        ):
            _blast7_to_data_frame(fp)
        fp = get_data_path('legacy9_invalid_too_many_columns')
        with self.assertRaisesRegex(
            BLAST7FormatError, r'Number of fields.*\(12\)'
        ):
            _blast7_to_data_frame(fp)
    ...
    def test_unrecognized_field_error(self):
        ...
        with self.assertRaisesRegex(
            BLAST7FormatError, r"Unrecognized field \(.*'sallid'\)"
        ):
            _blast7_to_data_frame(fp)
