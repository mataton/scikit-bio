...
from skbio.io.format.newick import (
    _newick_to_tree_node,
    _tree_node_to_newick,
    _newick_sniffer,
)
...
class TestNewick(unittest.TestCase):
    ...
    def _assert_equal(self, n1, n2):
        ...
        for c1, c2 in zip(
            sorted(n1.children, key=name), sorted(n2.children, key=name)
        ):
    ...
    def setUp(self):
        ...
        single_empty = (
            TreeNode(),
            [';\n', '[comment about the root' ' and its properties];'],
        )
        ...
        self.invalid_newicks = [
            ...
            (
                '\n'.join(
                    [','.join(str(i) for i in range(100)) for _ in range(100)]
                ),
                ['whitespace', 'label'],
            ),
        ]
