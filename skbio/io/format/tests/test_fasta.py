...
class ReaderTests(TestCase):
    def setUp(self):
        ...
        self.multi = (
            [
                ...
                Sequence(
                    'A',
                    metadata={'id': '_____seq__2_', 'description': ''},
                    positional_metadata={
                        'quality': np.asarray([42], dtype=np.uint8)
                    },
                ),
                Sequence(
                    'AACGGuA',
                    metadata={'id': '', 'description': 'desc3'},
                    positional_metadata={
                        'quality': np.asarray(
                ...
                Sequence(
                    ...
                    metadata={'id': '', 'description': ''},
                    positional_metadata={
                        'quality': np.asarray([10, 9, 8, 7, 6], dtype=np.uint8)
                    },
                ),
                Sequence(
                    'pQqqqPPQQQ',
        ...
        self.odd_labels_different_type = (
            [
                ...
                Protein(
                    'SKBI',
                    metadata={'id': '', 'description': 'skbio'},
                    positional_metadata={
                        'quality': np.asarray([1, 2, 33, 123], dtype=np.uint8)
                    },
                ),
            ],
        ...
        self.tabular_msa_different_type = (
            [
                RNA(
                    ...
                    metadata={'id': '', 'description': ''},
                    positional_metadata={
                        'quality': np.asarray([20, 20, 21], dtype=np.uint8)
                    },
                    lowercase='introns',
                ),
                RNA(
                    ...
                    metadata={
                        'id': 'rnaseq-1',
                        'description': 'rnaseq desc 1',
                    },
                    positional_metadata={
                        'quality': np.asarray([10, 9, 10], dtype=np.uint8)
                    },
                    lowercase='introns',
                ),
                RNA(
                    ...
                    metadata={
                        'id': 'rnaseq-2',
                        'description': 'rnaseq desc 2',
                    },
                    positional_metadata={
                        'quality': np.asarray([9, 99, 99], dtype=np.uint8)
                    },
                    lowercase='introns',
                ),
            ],
            {'constructor': partial(RNA, lowercase='introns')},
            list(map(get_data_path, ['fasta_tabular_msa_different_type'])),
            list(map(get_data_path, ['qual_tabular_msa_different_type'])),
        )
        ...
        self.lowercase_seqs = (
            [
                DNA(
                    ...
                    positional_metadata={
                        'quality': np.asarray([0, 1, 2, 3], dtype=np.uint8)
                    },
                    lowercase='introns',
                )
            ],
            {'constructor': DNA, 'lowercase': 'introns'},
            list(map(get_data_path, ['fasta_single_dna_seq_non_defaults'])),
            list(map(get_data_path, ['qual_single_dna_seq_non_defaults'])),
        )
        ...
        self.invalid_fps = list(
            map(
                lambda e: (get_data_path(e[0]), e[1], e[2], e[3]),
                [
        ...
        for exp, kwargs, fasta_fps, qual_fps in test_cases:
            for fasta_fp in fasta_fps:
                ...
                for qual_fp in qual_fps:
                    obs = list(
                        _fasta_to_generator(fasta_fp, qual=qual_fp, **kwargs)
                    )
    ...
    def test_fasta_to_any_sequence(self):
        for constructor, reader_fn in (
            (Sequence, _fasta_to_sequence),
            (
                partial(DNA, validate=False, lowercase='introns'),
                partial(_fasta_to_dna, validate=False, lowercase='introns'),
            ),
            (
                partial(RNA, validate=False, lowercase='introns'),
                partial(_fasta_to_rna, validate=False, lowercase='introns'),
            ),
            (
                partial(Protein, lowercase='introns'),
                partial(
                    _fasta_to_protein, validate=False, lowercase='introns'
                ),
            ),
        ):
            # empty file
            ...
            for fasta_fp in fasta_fps:
                ...
                exp.positional_metadata.insert(
                    ...
                    'quality',
                    np.asarray(
                        [10, 20, 30, 10, 0, 0, 0, 255, 1, 255], dtype=np.uint8
                    ),
                )
                qual_fps = list(
            ...
            for fasta_fp in fasta_fps:
                ...
                # get middle
                exp = constructor(
                    'ACGTTGCAccGG', metadata={'id': '', 'description': ''}
                )
    ...
    def test_fasta_to_tabular_msa(self):
        ...
        for exp_list, kwargs, fasta_fps, qual_fps in test_cases:
            ...
            for fasta_fp in fasta_fps:
                ...
                for qual_fp in qual_fps:
                    obs = _fasta_to_tabular_msa(
                        fasta_fp, qual=qual_fp, **kwargs
                    )
                    self.assertEqual(obs, exp)
...
class WriterTests(TestCase):
    def setUp(self):
        self.bio_seq1 = DNA(
            ...
            metadata={'id': 'seq1', 'description': 'desc1'},
            positional_metadata={
                'quality': [10, 20, 30, 10, 0, 0, 0, 255, 1, 255]
            },
            lowercase='introns',
        )
        self.bio_seq2 = DNA(
        ...
        self.dna_seq = DNA(
            'ACGTTGCAccGG',
            positional_metadata={
                'quality': [55, 10, 0, 99, 1, 1, 8, 77, 40, 10, 10, 0]
            },
            lowercase='introns',
        )
        self.rna_seq = RNA(
        ...
        self.prot_seq = Protein(
            ...
            positional_metadata={
                'quality': [42, 42, 255, 255, 42, 42, 42, 42, 42, 43]
            },
            lowercase='introns',
        )
        ...
        def mixed_qual_score_gen():
            ...
            yield DNA(
                'AAAAT',
                metadata={'id': 'da,dadadada', 'description': '10 hours'},
                lowercase='introns',
            )
        ...
        self.objs_fps = list(
            map(
                ...
                [
                    ...
                    (
                        single_seq_gen(),
                        {'max_width': 32, 'lowercase': 'introns'},
                        'fasta_single_seq',
                        'qual_single_seq',
                    ),
        ...
        self.invalid_objs = [
            ...
            (single_seq_gen(), {'max_width': 0}, ValueError, r'max_width=0'),
            (
                multi_seq_gen(),
                {'id_whitespace_replacement': '-\n_'},
                ValueError,
...
class RoundtripTests(TestCase):
    def test_roundtrip_generators(self):
        ...
        fps = list(
            map(
                lambda e: list(map(get_data_path, e)),
                [
                    ('empty', 'empty'),
        ...
        for fasta_fp, qual_fp in fps:
            ...
            qual_fh = io.StringIO()
            _generator_to_fasta(
                _fasta_to_generator(fasta_fp, qual=qual_fp),
                fasta_fh,
                qual=qual_fh,
            )
            obs_fasta = fasta_fh.getvalue()
    ...
    def test_roundtrip_biological_sequences(self):
        ...
        for reader, writer in (
            (_fasta_to_sequence, _sequence_to_fasta),
            (partial(_fasta_to_dna, validate=False), _dna_to_fasta),
            (partial(_fasta_to_rna, validate=False), _rna_to_fasta),
            (partial(_fasta_to_protein, validate=False), _protein_to_fasta),
        ):
            for fasta_fp, qual_fp in fps:
                # read
                obj1 = reader(fasta_fp, qual=qual_fp)
