...
from skbio.io.format.genbank import (
    _genbank_sniffer,
    _genbank_to_generator,
    _genbank_to_sequence,
    _genbank_to_dna,
    _genbank_to_rna,
    _genbank_to_protein,
    _parse_locus,
    _parse_reference,
    _generator_to_genbank,
    _sequence_to_genbank,
    _protein_to_genbank,
    _rna_to_genbank,
    _dna_to_genbank,
    _serialize_locus,
  ...
  JOURNAL   DNA Res. 6 (5), 283-290 (1999)
   PUBMED   10574454""".split('\n')
        ...
        exp = {
            ...
            'REFERENCE': '1  (bases 1 to 154478)',
            'TITLE': (
                'Complete structure of the chloroplast genome of'
                ' Arabidopsis thaliana'
            ),
        }
    ...
    def test_parse_locus_invalid(self):
        ...
        for line in lines:
            with self.assertRaisesRegex(
                GenBankFormatError, r'Could not parse the LOCUS line:.*'
            ):
                _parse_locus(line)
    ...
    def test_genbank_to_generator_single(self):
        ...
        for c in [Sequence, Protein]:
            obs = next(
                _genbank_to_generator(self.single_upper_fp, constructor=c)
            )
            exp = c(
                self.single[0],
...
class WriterTests(GenBankIOTests):
    ...
    def test_dna_protein_to_genbank(self):
        writers = [_protein_to_genbank, _dna_to_genbank]
        with io.StringIO() as fh:
            for i, (seq, md, imd, constructor) in enumerate(self.multi):
                obj = constructor(
